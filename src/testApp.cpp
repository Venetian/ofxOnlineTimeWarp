...
void testApp::keyPressed  (int key){
	...
	if (key == 'd')
		drawWarpMatrix = 1 - drawWarpMatrix;
	...
	if(setupFinished && args.deviceID == outputDeviceID){
		for (int i = 0; i < args.bufferSize; i++){
			if (outputIsSameAsInput){
				...
				//any old sine wave
				//args.buffer[i*2] = sin(sampleIndex / (2 * PI * 5.0));
				//args.buffer[i*2 + 1] = sin(sampleIndex / (2 * PI * 5.0));
				//sampleIndex++;
				args.buffer[i*2] = 0;//warpHolder->soundFileLoader->audioHolder.audioVector[audioListenerIndex*args.bufferSize+i];
				args.buffer[i*2 + 1] = 0;//warpHolder->soundFileLoader->audioHolder.audioVector[audioListenerIndex*args.bufferSize+i];
...
void testApp::showDeviceListWindow(ofxPortaudioSoundStream& paSoundStream){
	...
	ofSetColor(0,0,0);
	if (showingInputDevices)
		verdana14.drawString("select input", screenWidth - 300, deviceListHeightOffset);
	else 
		verdana14.drawString("select output", screenWidth - 300, deviceListHeightOffset);	
